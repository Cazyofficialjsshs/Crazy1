...
TOKEN = '7429271870:AAFggC4yp-mBHuGzf20H9cQp8GqzoLXG06s'
...
FORWARD_CHANNEL_ID = -1002148737530
CHANNEL_ID = -1002148737530
error_channel_id = -1002148737530
...
def process_attack_command(message):
    try:
        ...
        bot.send_message(message.chat.id, f"*🚀𝗔𝘁𝘁𝗮𝗰𝗸 𝗦𝗲𝗻𝘁 𝗦𝘂𝗰𝗰𝗲𝘀𝘀𝗳𝘂𝗹𝗹𝘆!🚀\n\n⏰𝗧𝗶𝗺𝗲: {duration}\n\n🔧𝗠𝗲𝘁𝗵𝗼𝗱: Kalajadu\n\n🔥𝗦𝘁𝗮𝘁𝘂𝘀: Attack In Progress.....🔥*", parse_mode='Markdown')
    except Exception as e:
...
def handle_message(message):
    if message.text == "Instant Plan 🧡":
        bot.reply_to(message, "*Instant Plan Buy Vip Ddos Server 💗*", parse_mode='Markdown')
    elif message.text == "Instant++ Plan 💥":
        bot.reply_to(message, "*Instant++ Plan selected Dm To Buy Vip Server Ddos @CrazyOfficials ❤️*", parse_mode='Markdown')
        attack_command(message)
    elif message.text == "Canary Download✔️":
        bot.send_message(message.chat.id, "*Please use the following link for Canary Download: https://t.me/CrazyVipSetupp/495*", parse_mode='Markdown')
        ...
        if user_data:
            ...
            response = "*No account information found. Please contact the administrator - @CrazyOfficials.*"
        bot.reply_to(message, response, parse_mode='Markdown')
    elif message.text == "Help❓":
        bot.reply_to(message, "*Help selected*", parse_mode='Markdown')
    elif message.text == "Contact admin✔️":
        bot.reply_to(message, "*Admin - @CrazyOfficials / Channel - @CrazyModOfficial*", parse_mode='Markdown')
    else:
